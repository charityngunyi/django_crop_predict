function NeedCropList2() {
    ...
    function xmlParser(xml) {
        ...
        $(xml).find("crop").each(function () {
            $("#output").append('<div class="book"><div class="title">' + $(this).find("cropname").text() + '</div><div class="description">' + $(this).find("category").text() + '</div><div class="date">Published ' + $(this).find("botanicalname").text() + '</div></div>');
            $("#output").fadeIn(1000);
        });
    }
    function failxml() {
...
function FilterCropList3(category) {
    ...
    function xmlParser(xml) {
        ...
        $(xml).find("crop").each(function () {
            ...
            if (category === the_category) {
                crop_view +="'"+crop+"'";
                $('#output').html('<div class="book"><div class="title"><a href="javascript:FilterCropName('+crop_view+')">'+crop+'</a></div><div class="description">' + $(this).find("category").text() + '</div><div class="date">Published ' + $(this).find("botanicalname").text() + '</div></div>');
                $("#output").fadeIn(1000);
            }
            else{
                sucess = 'error';
